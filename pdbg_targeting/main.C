...
void traverse(const TARGETING::TargetPtr& node)
{
    ...
    */
    TARGETING::AttributeTraits<TARGETING::ATTR_PHYS_DEV_PATH>::Type
        physDevPath{};
    if (node->tryGetAttr<TARGETING::ATTR_PHYS_DEV_PATH>(physDevPath))
    {
        std::string safePath(physDevPath,
                             strnlen(physDevPath, sizeof(physDevPath)));
...
int main()
{
    try
    {
        ...
        // printPropertiesOfNode(ts.getFDT(),
        // "//backplane0/proc_module0/hub_chip0");
        for (TARGETING::TargetPtr target : ts.getAllTargets())
        {
            TARGETING::AttributeTraits<TARGETING::ATTR_PHYS_DEV_PATH>::Type
                physDevPath{};
            std::memset(physDevPath, 0, sizeof(physDevPath));
            std::cout << "SIZE of  physDevPath " << sizeof(physDevPath)
                      << std::endl;
            if (target->tryGetAttr<TARGETING::ATTR_PHYS_DEV_PATH>(physDevPath))
            {
                std::string phyPathStr(physDevPath, sizeof(physDevPath));
                std::cout << "ATTR: " << phyPathStr << std::endl;
            }
            char fdtPath[256] = {};
            int rc = fdt_get_path(target->getFDT(), target->getOffset(),
                                  fdtPath, sizeof(fdtPath));
            if (rc >= 0)
            {
                std::cout << "FDT path: " << fdtPath << "\n";
            }
        }
    }
